...
export default function WorkingHoursCard() {
  ...
  const addSlot = () => setSlots([...slots, { from: "09:00", to: "17:00" }]);
    ...
    <div className="rounded-xl border border-slate-100 bg-white shadow-sm">
      <div className="p-6">
        <div className="mb-5 flex items-center justify-between">
          <h2 className="text-xl text-slate-900">{t("working_hours")}</h2>
          <span />
        </div>
        ...
        {slots.map((slot, index) => (
          <div key={index} className={index === 0 ? "" : "mt-5"}>
            <div className="grid grid-cols-1 gap-5 md:grid-cols-2">
              {/* من */}
              <div>
                <div className="mb-2 text-sm font-medium text-slate-700">{t("from")}</div>
              ...
              {/* إلى */}
              <div>
                <div className="mb-2 text-sm font-medium text-slate-700">{t("to")}</div>
                <div className="relative">
                  <span className="absolute right-4 top-1/2 -translate-y-1/2 text-emerald-500">
                    <FiClock className="h-5 w-5" />
                  </span>
